...
static void* rotary_encoder_running(void* arg)
{
    ...
    while (keepRunning) 
    {
        ...
        gpiod_line_request_both_edges_events((struct gpiod_line*)s_lineRotA, "Event waiting A");
        gpiod_line_request_both_edges_events((struct gpiod_line*)s_lineRotB, "Event waiting B");
        // Extract the actual gpiod_line* from your GpioLine struct

        // Wait for events on both lines in the bulk
        int numEvents = gpiod_line_event_wait_bulk(&bulkEvents, NULL, &bulkEvents);
                ...
                if (pCurrentState == &states[0]) {
                    ...
                    if (pSecondState && pSecondLastState) {
                        ...
                        // Counter clockwise check
                            ...
                            // printf("Counter clockwise rotation detected\n");
